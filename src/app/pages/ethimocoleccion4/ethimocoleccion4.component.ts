...
@Component({
  selector: "app-ethimocoleccion4",
  templateUrl: "./ethimocoleccion4.component.html",
})
export class Ethimocoleccion4Component implements OnInit {
  articulos: ArticulocoleccionesModel[] = [];
  idenlace: number = 30;
  idtipo: number = 5;
  idtipotexto: number = 7;
  idcoleccion: number = 4;
  ...
  getArticulos() {
    this.articulocoleccionesService
      .getarticulocoleccionesGallery(this.idcoleccion)
      .subscribe((resp: ArticulocoleccionesModel[]) => {
        if (resp) {
          this.articulos = resp;

          this.articulos.forEach((el) => {
            el.articulosacabados1 = el.articulosacabados1.filter(
              (x) => x.acabadocategorias_id == 1
            );
            el.articulosacabados2 = el.articulosacabados2.filter(
              (x) => x.acabadocategorias_id == 2
            );
            el.articulosacabados3 = el.articulosacabados3.filter(
              (x) => x.acabadocategorias_id == 3
            );
          });
